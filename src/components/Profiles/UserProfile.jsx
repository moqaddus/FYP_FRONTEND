...
const UserProfile = () => {
  ...
  const handleDeleteInterest = async (interestId) => {
    try {
      ...
      const response = await axios.delete(`${import.meta.env.VITE_API_KEY}/interest/delete/${tokenId}/${interestId}`);
      await fetchUserInterestNames();
      await fetchInterestsNotOfUser();
      // }
    } catch (error) {
  ...
  if (error) {
    return (
      <div className="mt-[200px] mb-[200px] text-lg text-center font-bold font-mono">
  ...
  if (!userData) {
    return (
      <div className="mt-[200px] mb-[200px] text-lg text-center font-mono">
  ...
  return (
    <div className="md:flex">
      ...
      <div>
        <div
          className={`fixed top-0 left-0 w-1/4 h-full bg-gray-700 bg-opacity-75 transition-all duration-300 ease-out z-10 ${
            showChats ? "translate-x-0" : "-translate-x-full"
          }`}
        >
          <button
            onClick={toggleChats}
            className="absolute top-4 right-4 bg-gray-200 hover:bg-gray-300 text-gray-800 px-2 py-1 rounded-full"
          >
            Close
          </button>
          <div className="h-full flex items-center justify-center">
            <ul className="p-4 space-y-4 w-full">
              {chats.map((chat) => (
                <li
                  key={chat._id}
                  className="py-2 flex items-center space-x-4 bg-white bg-opacity-90 shadow-md hover:bg-opacity-100 transition-all duration-200 rounded-md p-4"
                >
                  <img
                    src={chat.userImage}
                  ...
                  />
                  <Link
                    to={{ pathname: "/chatHome", state: { user2: chat._id } }}
        ...
        <div className="md:bg-gray-800 md:mt-24 md:text-white md:w-64 md:min-h-screen hidden md:block">
          <ul className="p-4">
            ...
            {showEdit && (
              <li className="py-2 pl-1 hover:bg-gray-600 rounded-lg">
            ...
            {showEdit && (
              <li className="py-2 pl-1 hover:bg-gray-600 rounded-lg">
                <Link to={`/pendingReview/${tokenId}`}>Review Events</Link>
              </li>
            )}
            {showEdit && (
              <li
            ...
            {isOrganization && (
              <li className="py-2 pl-1 hover:bg-gray-600 rounded-lg">
                <Link to={{ pathname: "/chatHome", state: { user2: userId } }}>
                  Message
                </Link>
              </li>
            )}
      ...
      <div className="max-w-4xl mx-auto mt-24 p-4 bg-gray-100 rounded-lg shadow-lg">
        <div className="flex flex-col md:flex-row items-center justify-between mb-4">
          ...
          <div className="md:ml-auto md:text-right md:mt-[80px] flex items-center">
            {showEdit && (
              <button className="bg-orange-400 hover:bg-orange-500 text-white px-4 py-2 rounded-lg shadow-md">
                <Link
                  to={{
                    pathname: "/editUser",
            ...
            {isOrganization && (
              <button className="md:hidden bg-orange-400 hover:bg-orange-500 text-white px-4 py-2 rounded-lg shadow-md mr-2">
                {" "}
        ...
        <div className="mb-4 relative">
          ...
          <div className="flex flex-wrap mb-2">
            {userInterest.map((interest) => (
              <span
                key={interest._id} value={interest._id}
                // className="bg-green-500 hover:bg-green-400 text-white rounded-full px-2 py-1 mr-2 mb-2"
                className={`${
                  showEdit ? 'bg-green-500 hover:bg-red-700 transition duration-[1000ms]' : 'bg-green-500 hover:bg-green-400 transition duration-[1000ms]'
                } text-white rounded-full px-2 py-1 mr-2 mb-2 cursor-pointer`}
                onClick={() => showEdit && handleDeleteInterest(interest._id)}
              >
